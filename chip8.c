...
static inline byte c8_key_released(Chip8 *const chip8) {
	for (byte k = 0; k < NUM_KEYS; k++) {
		if (chip8->keys_prev[k] && !chip8->keys[k]) return k;
	}
	return NUM_KEYS;
}

// returns true if there is a collision between a sprite pixel and a screen pixel that are both on
static inline bool draw_sprite(Chip8 *const chip8, const byte height, const byte vx, const byte vy, const word sprite_addr) {
	bool collision = false;
	// iterate through each row of the sprite (i.e., each byte)
	for (int row = 0; row < height; row++) {
		byte sprite_byte = chip8->memory[sprite_addr + row];
...
void c8_execute(Chip8 *const chip8, const word opcode) {
	...
	switch (a) {
		...
		case 0xD:
			if (draw_sprite(chip8, n, v[x], v[y], chip8->index)) v[0xF] = 1;
			break;
